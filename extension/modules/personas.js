...
var PersonaController = {
    ...
    _createPersonaItem: function(persona,document) {
        ...
        item.setAttribute("autocheck", "false");
        item.addEventListener("command",function(event){PersonaController.onSelectPersona(event)},true);
    ...
    _createViewMoreItem: function(category, categoryId, document) {
        ...
        item.addEventListener("command",function(event){PersonaController.openURLInTab(this.getAttribute('href'),event)},true);
	...
	createToolbarButton:function(document){
		...
		toolbarbutton.setAttribute("class", (PersonaController.appInfo.ID!=PersonaController.SEAMONKEY_ID) ? "toolbarbutton-1" : "toolbarbutton-1 seamonkey");
	...
	addToolbarButton:function(window){
		...
		if (window.CustomizableUI) {
			...
			if(PersonaController.appInfo.ID!="{8de7fcbb-c55c-4fbe-bfc5-fc555c87dbc4}") PersonaController.placeToolbarButton(window,toolbarbutton);
			else{
	...
	addToolsMenu:function(window){
		...
		personasselectormenu.addEventListener("popuphiding",function(event){PersonaController.onPopupHiding(event)},true);
		personasselectormenu.addEventListener("popupshowing",function(event){return PersonaController.onPopupShowing(event)},true);
		...
		personaspluscustommenuapply.addEventListener("command",function(event){PersonaController.onSelectPersona(event)},true);
		...
		menuitem.addEventListener("command",function(event){PersonaController.onEditCustomPersona(event)},true);
		...
		defaultpersona.addEventListener("command",function(event){PersonaController.onSelectPersona(event)},true);
		...
		menuitem_0.addEventListener("command",function(event){PersonaController.onSelectPreferences(event)},true);
		...
		personasselectorbutton.addEventListener("mousedown",function(event){PersonaController.onMenuButtonMouseDown(event)},true);
